    ...
    <div className="content fade-up">
            ...
            <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom: 16}}>
              <div>
                ...
                <div className="t-display" style={{fontSize: 48, fontWeight: 700, color:"var(--accent)", lineHeight: 1, marginTop: 4}}>
                  {62 + (tick % 3)}<span style={{fontSize: 22, color:"var(--fg-3)"}}>%</span>
                </div>
                <div className="t-mono green" style={{fontSize: 11, marginTop: 4}}>↑ +4% last 60s</div>
              </div>
              <window.Donut value={62} size={100} stroke={10} label="MOMENTUM"/>
            </div>
            <div className="overlay-winrate-bar" style={{height: 28}}>
              <div className="overlay-winrate-blue" style={{width: "62%"}}>62%</div>
              <div className="overlay-winrate-red">38%</div>
            </div>
          ...
          <div className="panel-body" style={{display:"flex", flexDirection:"column", gap: 10}}>
            <div style={{display:"flex", gap: 12, padding: 14, background:"var(--accent-soft)", border:"1px solid var(--accent)", borderRadius: 8}}>
              <div style={{
                ...
                fontFamily:"var(--ff-display)", fontSize: 22, fontWeight: 700, color:"#0a0613",
                flexShrink: 0
              }}>1</div>
              <div>
            ...
            <div style={{display:"flex", gap: 10, padding: 10, background:"var(--bg-3)", borderRadius: 6}}>
              <div style={{width: 24, height: 24, borderRadius: 4, background:"var(--bg-4)", display:"grid", placeItems:"center", fontFamily:"var(--ff-display)", fontWeight:700, fontSize: 12, color:"var(--fg-2)"}}>2</div>
              <div style={{fontSize: 12, color:"var(--fg-1)", lineHeight: 1.5}}>Place deep ward at enemy red buff before contesting</div>
            </div>
            <div style={{display:"flex", gap: 10, padding: 10, background:"var(--bg-3)", borderRadius: 6}}>
              <div style={{width: 24, height: 24, borderRadius: 4, background:"var(--bg-4)", display:"grid", placeItems:"center", fontFamily:"var(--ff-display)", fontWeight:700, fontSize: 12, color:"var(--fg-2)"}}>3</div>
              <div style={{fontSize: 12, color:"var(--fg-1)", lineHeight: 1.5}}>Trade 2nd Drake for Herald — your team has +500 gold lead</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid-2">
        <div className="panel">
          ...
          <div className="panel-body" style={{display:"flex", flexDirection:"column", gap: 10}}>
            {[
              { lane: "TOP", you: "Halberd", them: "Ironwake", state: "Even, frozen at river", color: "var(--fg-2)" },
              { lane: "MID", you: "Noctis", them: "Echo", state: "Pushed by you · push prio", color: "var(--green)" },
              { lane: "BOT", you: "Quill+Myra", them: "Talon+Verdant", state: "Crashed under enemy turret", color: "var(--red)" },
            ].map(l => (
        ...
        <div className="panel">
          ...
          <div className="panel-body" style={{display:"flex", flexDirection:"column", gap: 8}}>
            {[
              { id: "echo", spell: "Spectral Drift", cd: "12s", color: "var(--green)" },
              { id: "thornroot", spell: "Earthen Ult", cd: "READY", color: "var(--red)" },
              { id: "verdant", spell: "Hookline", cd: "44s", color: "var(--fg-2)" },
              { id: "talon", spell: "Burst Step", cd: "8s", color: "var(--amber)" },
            ].map(c => (
              <div key={c.id} style={{display:"grid", gridTemplateColumns: "32px 1fr auto", gap: 10, alignItems:"center"}}>
                <window.Champ id={c.id} size="sm" withTooltip/>
                <div>
                  <div style={{fontSize: 12, fontWeight: 500}}>{window.champById(c.id).name}</div>
...
window.MacroScreen = MacroScreen;
