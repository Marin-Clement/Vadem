...
fn base_cd(raw_spell: &str, level: u32) -> f64 {
    if raw_spell.contains("Teleport") {
        ...
        if level >= 10 { 240.0 } else { 360.0 - (level as f64 - 1.0) * 13.3 }
    } else if raw_spell.contains("Flash")    { 300.0 }
    else if raw_spell.contains("Ignite")     { 210.0 }
    else if raw_spell.contains("Exhaust")    { 210.0 }
    else if raw_spell.contains("Heal")       { 210.0 }
    else if raw_spell.contains("Haste")      { 210.0 } // Ghost
    else if raw_spell.contains("Boost")      { 210.0 } // Cleanse
    else if raw_spell.contains("Barrier")    { 210.0 }
    else if raw_spell.contains("Smite")      { 15.0 }
    else if raw_spell.contains("Dot")        { 210.0 } // raw Ignite variant
    else { 210.0 }
}

fn apply_haste(cd: f64, item_ids: &[u32]) -> f64 {
    let haste: u32 = item_ids.iter()
...
#[tauri::command]
pub fn mark_spell_used(
    req: MarkSpellRequest,
    registry: State<'_, SpellRegistry>,
) {
    let cd = apply_haste(base_cd(&req.raw_spell, req.level), &req.item_ids);
    let key = format!("{}#{}", req.summoner_name, req.slot);
    let mut map = registry.lock().unwrap();
    map.insert(key, CdEntry { used_at: SystemTime::now(), base_cd: cd });
}

#[tauri::command]
pub fn get_spell_cooldowns(
    registry: State<'_, SpellRegistry>,
) -> Vec<CooldownInfo> {
