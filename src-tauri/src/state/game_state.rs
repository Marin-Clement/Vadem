...
fn scaling_map() -> &'static HashMap<String, f32> {
    SCALING.get_or_init(|| {
        serde_json::from_str(include_str!("../../champ_scaling.json"))
            .unwrap_or_default()
    })
}
...
fn event_team(evt: &GameEvent, players: &[AllPlayer]) -> Option<String> {
    if !evt.killer_name.is_empty() {
        if let Some(p) = players.iter().find(|p| names_match(&p.summoner_name, &evt.killer_name)) {
            return Some(p.team.clone());
        }
    }
    for assister in &evt.assisters {
        if let Some(p) = players.iter().find(|p| names_match(&p.summoner_name, assister)) {
            return Some(p.team.clone());
        }
    }
    None
...
pub fn build_payload(data: &AllGameData, my_summoner: &str) -> GameStatePayload {
    ...
    let events: &[GameEvent] = data.events.as_ref()
        .and_then(|e| e.events.as_deref())
        .unwrap_or_default();

    // ── Team identity ────────────────────────────────────────────────────────
    let my_team = players_raw.iter()
    ...
    let sum_kills = |ps: &[&AllPlayer]| -> u32 {
        ps.iter().filter_map(|p| p.scores.as_ref()).map(|s| s.kills).sum()
    };
    let order_kills = sum_kills(&order_players);
    let chaos_kills  = sum_kills(&chaos_players);
    let ally_kills   = if my_is_order { order_kills } else { chaos_kills };
    let enemy_kills  = if my_is_order { chaos_kills } else { order_kills };

    // ── Levels + CS ──────────────────────────────────────────────────────────
    let order_levels: u32 = order_players.iter().map(|p| p.level).sum();
    let chaos_levels: u32  = chaos_players.iter().map(|p| p.level).sum();
    let order_cs: u32 = order_players.iter().filter_map(|p| p.scores.as_ref()).map(|s| s.creep_score).sum();
    let chaos_cs: u32  = chaos_players.iter().filter_map(|p| p.scores.as_ref()).map(|s| s.creep_score).sum();

    // ── Ward score ───────────────────────────────────────────────────────────
    let order_wards: f64 = order_players.iter().filter_map(|p| p.scores.as_ref()).map(|s| s.ward_score).sum();
    let chaos_wards: f64  = chaos_players.iter().filter_map(|p| p.scores.as_ref()).map(|s| s.ward_score).sum();

    // ── Item value & carry gold diff ─────────────────────────────────────────
    let player_item_value = |p: &AllPlayer| -> u32 {
        p.items.as_deref().unwrap_or_default().iter().map(|i| i.price).sum::<u32>()
    };
    ...
    let item_value_diff = order_vals.iter().sum::<u32>() as f32 - chaos_vals.iter().sum::<u32>() as f32;
    ...
    let tier_weight = |count: u32| -> f32 { match count { 0 => 3.0, 1 => 5.0, 2 => 7.0, _ => 9.0 } };
    ...
    for t in events.iter().filter(|e| e.event_name == "TurretKilled") {
        ...
        let lane = t.turret_killed.split('_')
            .find(|s| s.len() == 2 && s.starts_with('L')
    ...
    let chaos_inhibs: usize = events.iter()
        .filter(|e| e.event_name == "InhibKilled"
            && e.inhib_killed.to_ascii_lowercase().contains("tchaos")
            && game_time - e.event_time < 300.0)
        .count();
    let order_inhibs: usize = events.iter()
        .filter(|e| e.event_name == "InhibKilled"
    ...
    // ── Elder ────────────────────────────────────────────────────────────────
    let blue_elder_active = events.iter()
        .any(|e| e.event_name == "DragonKill" && e.dragon_type == "Elder"
            && event_team(e, players_raw).as_deref() == Some("ORDER")
            && game_time - e.event_time < 150.0);
    let red_elder_active = events.iter()
        .any(|e| e.event_name == "DragonKill" && e.dragon_type == "Elder"
            && event_team(e, players_raw).as_deref() == Some("CHAOS")
            && game_time - e.event_time < 150.0);

    // ── Baron ────────────────────────────────────────────────────────────────
    let blue_baron_active = events.iter()
        .any(|e| e.event_name == "BaronKill"
            && event_team(e, players_raw).as_deref() == Some("ORDER")
            && game_time - e.event_time < 180.0);
    let red_baron_active = events.iter()
        .any(|e| e.event_name == "BaronKill"
            && event_team(e, players_raw).as_deref() == Some("CHAOS")
            && game_time - e.event_time < 180.0);

    // ── Grubs ────────────────────────────────────────────────────────────────
    let order_grubs = events.iter()
        .filter(|e| e.event_name == "HordeKill"
            && event_team(e, players_raw).as_deref() == Some("ORDER"))
        .count();
    let chaos_grubs = events.iter()
        .filter(|e| e.event_name == "HordeKill"
            && event_team(e, players_raw).as_deref() == Some("CHAOS"))
        .count();
    ...
    let top_scaling_diff    = (team_scaling(&order_players, "TOP")     - team_scaling(&chaos_players, "TOP"))     * time_weight;
    let jungle_scaling_diff = (team_scaling(&order_players, "JUNGLE")  - team_scaling(&chaos_players, "JUNGLE"))  * time_weight;
    let mid_scaling_diff    = (team_scaling(&order_players, "MIDDLE")  - team_scaling(&chaos_players, "MIDDLE"))  * time_weight;
    let adc_scaling_diff    = (team_scaling(&order_players, "BOTTOM")  - team_scaling(&chaos_players, "BOTTOM"))  * time_weight;
    let sup_scaling_diff    = (team_scaling(&order_players, "UTILITY") - team_scaling(&chaos_players, "UTILITY")) * time_weight;
    ...
    let to_summary = |p: &AllPlayer| PlayerSummary {
        ...
        spell1_raw:     p.summoner_spells.as_ref()
                         .and_then(|s| s.spell_one.as_ref())
                         .map(|s| s.raw_name.clone())
                         .unwrap_or_default(),
        spell1_display: p.summoner_spells.as_ref()
                         .and_then(|s| s.spell_one.as_ref())
                         .map(|s| s.display_name.clone())
                         .unwrap_or_default(),
        spell2_raw:     p.summoner_spells.as_ref()
                         .and_then(|s| s.spell_two.as_ref())
                         .map(|s| s.raw_name.clone())
                         .unwrap_or_default(),
        spell2_display: p.summoner_spells.as_ref()
                         .and_then(|s| s.spell_two.as_ref())
                         .map(|s| s.display_name.clone())
                         .unwrap_or_default(),
        item_ids:       p.items.as_deref()
                         .unwrap_or_default()
                         .iter()
                         .map(|i| i.item_id)
                         .collect(),
